...
public class ReadStory implements Serializable {
    private String madlipsText;                 // text of the story
    ...
    public ReadStory(InputStream stream) {
        read(stream);
    }

    /** constructs a new Story reading its text from the given Scanner */
    public ReadStory(Scanner input) {
        read(input);
    }
    ...
    public void fillInPlaceholder(String word) {
        if (!isFilledIn()) {
            madlipsText = madlipsText.replace("<" + filledIn + ">", word);
            filledIn++;
        }
    }
    ...
    public void read(Scanner input) {
        while (input.hasNext()) {
            ...
            if (word.startsWith("<") && word.endsWith(">")) {
                ...
                if (htmlMode) {
                    madlipsText += " <b><" + placeholders.size() + "></b>";
                } else {
                    madlipsText += " <" + placeholders.size() + ">";
                }
